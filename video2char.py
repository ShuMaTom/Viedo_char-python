# coding:UTF-8
# for python class pre
import os
import cv2
from PIL import Image, ImageFont, ImageDraw
from cv2 import VideoWriter, VideoWriter_fourcc, imread, resize      # OpenCV


def process_bar(percent, start_str='', end_str='', total_length=0):  # 进度条
...
def video_to_pic(vp):                                               # 将视频转换为图片 并进行计数 返回总共生成了多少张图片
    number = 0
    if vp.isOpened():                                               # vp = cv2.VideoCapture(video_path)
        r, frame = vp.read()
...
def img_to_char(image_path, raw_width, raw_height, task):
    ...
    for i in range(height):
        for j in range(width):
            pixel = im.getpixel((j, i))                   # 获取（j，i）处grb颜色值
            color.append((pixel[0], pixel[1], pixel[2]))  # 将颜色加入进行索引
            if len(pixel) == 4:
                txt += get_char(pixel[0], pixel[1], pixel[2], pixel[3])
            else:
                txt += get_char(pixel[0], pixel[1], pixel[2])   # 将（j，i）坐标的 RGB 像素转为字符后添加到 txt 字符串
        txt += '\n'
...
if __name__ == '__main__':
    video_path = '192597401-1-112.flv'
